...
include("connect.php");
 ...
 $result1=mysql_query("REPLACE INTO present_stock (name, type, batch,quantity,exp,present)
SELECT stock.name, stock.type, stock.batch_no,stock.quantity,stock.exp,stock.quantity - (select COALESCE( sum(details.quantity),0) from details where stock.batch_no=details.batch_no)  from stock   ");
 if($result1)
 {
 	echo mysql_error();
 }
  
   $result11=mysql_query("REPLACE INTO stock_current (NAME,TYPE,batch,quantity,EXP,current)
SELECT present_stock.name,present_stock.type,present_stock.batch,present_stock.quantity,present_stock.exp,
present_stock.present -(SELECT COALESCE (SUM(transfer.quantity),0) FROM transfer WHERE present_stock.batch LIKE transfer.batch_no) AS kk 
FROM present_stock");
 if(!$result11)
 {
 	echo mysql_error();
...
//$result111=mysql_query("REPLACE INTO Final_stock (last_name, first_name)
  // -> VALUES( 'Ajay', 'Kumar');
 /*result=mysql_query("SELECT stock.quantity - (select sum(details.quantity) from details where stock.name=details.medicine) AS sub,
 stock.id AS id,
 stock.name AS name,
 stock.type AS type,
...
//$result=mysql_query("select * from present_stock");
 ?>
 <html>
<head>
	<title>Invoice</title>
...
<div class="dropdown">
  ...
  <div class="dropdown-content">
	...
	<table class="data-table"><br> <br>
		<caption class="title">List Of Stock <a href="record.php">Click here for check the recorder level</a> <br> <a href="print_sr.php" style="color: red;">(Print)</a><-------><a href="ex_sr.php" style="color: red;">(Download Excel File)</a></caption>
		<thead>
			<tr>
				<th>ID</th>
				<th>MEDICINES</th>
