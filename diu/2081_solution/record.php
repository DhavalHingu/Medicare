...
$result1=mysql_query("REPLACE INTO recorder (medicine,total,batch,current)
SELECT medicines.medicine,medicines.level,medicines.batch,medicines.level - (select COALESCE( sum(details.quantity),0) from details where medicines.batch=details.batch_no)  from medicines ");
 if($result1)
 {
 	echo mysql_error();
 }
 ...
 $result11=mysql_query("REPLACE INTO recorder_current (medicine,total,batch,current)
SELECT recorder.medicine,recorder.total,recorder.batch,recorder.current -(SELECT COALESCE (SUM(transfer.quantity),0) FROM transfer WHERE recorder.batch LIKE transfer.batch_no) AS kk FROM recorder");
 if(!$result11)
 {
 	echo mysql_error();
 }
...
$result=mysql_query("select * from `recorder_current`");
...
// $result1=mysql_query("REPLACE INTO recorder (medicine,total,batch,current)
//SELECT medicines.medicine,medicines.level,medicines.batch,medicines.level - (select COALESCE (sum(stock_current.current),0) from stock_current where medicines.batch=stock_current.batch)  FROM medicines");
 //if(!$result1)
 //{
 	//echo mysql_error();
 //}
...
<div class="dropdown">
  ...
  <div class="dropdown-content">
	...
	<table class="data-table">
		<caption class="title">Recorder Level <a href="sr.php">Click here for check the stock report</a>  <br> <a href="print_record.php" style="color: red;">(Print)</a><-------><a href="ex_record.php" style="color: red;">(Download Excel File)</a></caption>
		<thead>
			<tr>
				<th>ID</th>
				<th>MEDICINES</th>
