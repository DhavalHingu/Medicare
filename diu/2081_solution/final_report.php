...
include("connect.php");
 ...
 $result1=mysql_query("INSERT INTO recorder (medicine,total,current)
SELECT medicines.medicine,medicines.level,medicines.level - (select sum(details.quantity) from details where medicines.medicine=details.medicine)  FROM medicines group by medicines.medicine on ON DUPLICATE KEY UPDATE recorder.medicine=VALUES(medicine),recorder.total=values(total),recorder.current=values(current)  ");
 if(!$result1)
 {
 	echo mysql_error();
 }

 $result11=mysql_query("INSERT INTO recorder_current (medicine,total,current)
SELECT recorder.medicine,recorder.total,recorder.current - (select COALESCE (sum(transfer.quantity),0) from transfer where recorder.medicine=transfer.medicine) FROM recorder group by recorder.medicine ON DUPLICATE KEY UPDATE recorder_current.medicine=VALUES(medicine),recorder_current.total=values(total),recorder_current.current=values(current) ");
 if(!$result11)
 {
 	echo mysql_error();
 }
...
$sql = "SELECT * FROM recorder_current ORDER BY id ASC LIMIT $start_from, $limit";  
...
<div class="dropdown">
  ...
  <div class="dropdown-content">
	...
	<table class="data-table">
		<caption class="title">Recorder Level <a href="sr.php">Click here for check the stock report</a></caption>
		<thead>
			<tr>
				<th>ID</th>
				<th>MEDICINES</th>
				<th>recorder level</th>
				<th>current recorder level</th>
				</tr>
		</thead>
		<tbody>
		<?php
...
echo "<td>" .$row['id']."</a></td>";
	...
	<?php  
$sql = "SELECT COUNT(id) FROM recorder_current";  
$rs_result = mysql_query($sql);  
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li><a href='final_report.php?page=".$i."'>".$i."</a></li>";  
};  
echo $pagLink . "</ul></nav>";  
?>
...
$('.pagination').pagination({
		...
		hrefTextPrefix : 'final_report.php?page='
    });
	});
</script>
